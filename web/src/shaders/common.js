...
const vec4 DOMAIN = vec4(-24.0, -24.0, 48.0, 48.0);
...
const float BEDROCK_EXTENT = 40.0;
...
/// The working ground: where the shore is flat and where anything you build is
/// worth counting. One function, so those cannot drift apart.
...
/// The hardpack. Two headlands rather than the native build's five — this is
/// the minimal edition, and two is enough to frame a shot.
float bedrock(vec2 p) {
    float z = p.y;
    float y = 1.55 - 0.0705 * (z + 24.0);

    // Landward dune ridge, pushed out past the working ground so the camera can
    // pull back over it without burying itself in a hill.
...
float rockiness(vec2 p) {
    return max(domeMask(p, vec2(-22.6, 7.5), 5.4),
               domeMask(p, vec2(23.8, -1.5), 4.8));
}

/// The loose sand last night's tide left behind.
float sandBed(vec2 p) {
    float pad = buildPad(p);
    // The hardpack dropped 0.98 m under the same pad, so the surface lands where
    // it always did — but there is now the better part of two metres of sand
    // over it, which is enough to cut a moat you could lose a spade in.
    float d = 0.30 + 1.30 * pad + 0.18 * vnoise(p * 0.33);
